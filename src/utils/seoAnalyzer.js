...
export function extractKeywords(title, description = '') {
  ...
  const stopWords = new Set(['that','this','with','from','they','have','what','been','were','will','would','could','should','their','about','which','when','your','some','into','more','also','then','than','very','just','like','most','over','such','only','both','each','much','many','time','used','using','these','those','make','made','take','does','want','know','need','here','there','even','other','well','said','same','next','back','good','give','come','look','after','before','where','still','being','every','being','while','under','never','might','must','also'])
...
export function analyzeSEO(videoData) {
  ...
  if (titleLen >= 40 && titleLen <= 70) {
    ...
    titleScore += 4
    issues.push({ type: 'warning', category: 'Title', msg: `Title is short (${titleLen} chars). Aim for 40–70 characters for best search visibility.` })
  } else if (titleLen > 70) {
    titleScore += 4
    issues.push({ type: 'warning', category: 'Title', msg: `Title is long (${titleLen} chars). YouTube truncates at ~70 chars in search results.` })
  } else {
    issues.push({ type: 'error', category: 'Title', msg: `Title is very short. This severely limits discoverability.` })
  }
  ...
  if (descWords >= 150) {
    ...
    descScore += 4
    issues.push({ type: 'warning', category: 'Description', msg: `Description has only ${descWords} words. Aim for 150–300 words for optimal SEO.` })
  } else if (descWords > 0) {
    descScore += 1
    issues.push({ type: 'error', category: 'Description', msg: `Description is very thin (${descWords} words). YouTube uses description text for search indexing.` })
  } else {
    issues.push({ type: 'error', category: 'Description', msg: 'No description provided. This is a critical SEO gap.' })
  }
  ...
  if (keywordInOpening) {
    ...
    issues.push({ type: 'warning', category: 'Description', msg: 'Primary keyword not found in the first 150 characters of description (the "above the fold" preview area).' })
  ...
  if (hasCTA) {
    ...
    suggestions.push('Add a clear CTA: "Subscribe for weekly insights", "Drop a comment below", and "Visit [link] to learn more".')
  }
  ...
  if (hasTimestamps) {
    ...
    suggestions.push('Add video chapters with timestamps (e.g., 0:00 Intro, 2:30 Main Topic) to improve user experience and SEO.')
  }
  ...
  if (hasLinks) {
    ...
    issues.push({ type: 'warning', category: 'Description', msg: 'No external links found. Including relevant links (website, resources) improves credibility.' })
  }

  scores.description = Math.min(descScore, 25)

  // --- Hashtags & Tags (20 pts) ---
  let tagScore = 0

  if (hashtags.length > 0) {
    tagScore += 8
    if (hashtags.length < 3) {
      issues.push({ type: 'warning', category: 'Hashtags', msg: `Only ${hashtags.length} hashtag(s) found. Use 3–15 hashtags for maximum discoverability.` })
      tagScore -= 3
    } else if (hashtags.length > 15) {
      issues.push({ type: 'warning', category: 'Hashtags', msg: `${hashtags.length} hashtags found. YouTube may ignore hashtags beyond 15; keep it focused.` })
      tagScore -= 2
    }
  } else {
    issues.push({ type: 'error', category: 'Hashtags', msg: 'No hashtags found in description. Hashtags significantly boost discoverability and categorization.' })
  ...
  if (tags.length >= 5) {
    ...
    tagScore += 3
    issues.push({ type: 'warning', category: 'Tags', msg: `Only ${tags.length} tag(s) set. Use 5–15 relevant tags to maximize search reach.` })
  } else {
    issues.push({ type: 'error', category: 'Tags', msg: 'No video tags found. Tags help YouTube categorize your content.' })
    suggestions.push('Add 8–12 tags covering broad and specific terms related to your video topic.')
  }
  ...
  if (captions) {
    ...
    issues.push({ type: 'warning', category: 'Accessibility', msg: 'No captions/subtitles detected. Captions improve accessibility and can boost SEO by up to 12%.' })
    suggestions.push('Upload a manually corrected transcript or enable auto-captions for better accessibility and indexing.')
  }

  if (contentDetails?.definition === 'hd') {
    techScore += 3
  } else {
    issues.push({ type: 'warning', category: 'Technical', msg: 'Video is not HD. High-definition videos rank better and have higher viewer retention.' })
  }

  techScore += 4 // assume custom thumbnail for analyzed videos
  ...
  if (monthsOld <= 12) {
    ...
    suggestions.push('Consider refreshing this older video with an updated thumbnail, title, and description to re-capture search rankings.')
  }
  ...
  if (extractedKws.length > 0) {
    ...
    if (titleKws.length < 2) {
      keywordIssues.push('Primary and secondary keywords are not prominent in the title.')
    }
    if (!keywordInOpening) {
      keywordIssues.push('Keywords are not placed in the critical "above the fold" area (first 150 chars) of the description.')
    }
  ...
  return {
    ...
    internalLinkingTips,
    missingElements: buildMissingElements(hashtags, tags, captions, hasCTA, hasTimestamps, descWords, titleLen),
  }
}

function buildMissingElements(hashtags, tags, captions, hasCTA, hasTimestamps, descWords, titleLen) {
  const elements = [
    ...
    { label: 'External Links in Description', present: false },
  ]
  return elements
}
...
function generateLongTailKeywords(title, keywords) {
  const modifiers = [
    'for beginners', 'tutorial', 'step by step', 'explained', 'in 2024', 'in 2025',
    'how to', 'best practices', 'tips and tricks', 'complete guide', 'case study',
    'for executives', 'B2B strategy', 'ROI analysis', 'advanced techniques',
  ]
