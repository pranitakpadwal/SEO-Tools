...
import { Youtube, Search, Sparkles, Scissors, BarChart2, ChevronRight, AlertCircle } from 'lucide-react'
...
export default function App() {
  ...
  const handleAnalyze = useCallback(async (url) => {
    if (!apiKey) { setError('Please enter your YouTube Data API key first.'); return }
    ...
    try {
      ...
      const titles = generateTitleVariations(
        data.snippet?.title || '',
        seo.extractedKeywords
      )
      const desc = generateDescription(
        data.snippet?.title || '',
        seo.extractedKeywords,
        data.snippet?.channelTitle || 'Your Channel'
      )
  ...
  const handleAnalyzeChannel = useCallback(async (url) => {
    ...
    try {
      const info = await fetchChannelInfo(channelId, apiKey)
      setChannelInfo(info)

      const realChannelId = info.id
      const videos = await fetchVideosByChannel(realChannelId, apiKey)
