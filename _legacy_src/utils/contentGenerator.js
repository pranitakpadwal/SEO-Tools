...
export function generateTitleVariations(originalTitle, keywords) {
  ...
  const variations = [
    { type: 'SEO', badge: 'badge-seo', title: `${cap(kw1)} ${cap(kw2)}: Complete Guide [${year}]` },
    { type: 'SEO', badge: 'badge-seo', title: `How to Master ${cap(kw1)}: Proven ${cap(kw2)} Framework` },
    { type: 'SEO', badge: 'badge-seo', title: `${cap(kw1)} Explained: Everything You Need to Know` },
    { type: 'CTR', badge: 'badge-ctr', title: `7 ${cap(kw1)} Mistakes Killing Your ${cap(kw2)} (Fix These Now)` },
    { type: 'CTR', badge: 'badge-ctr', title: `I Tested ${cap(kw1)} for 90 Days — Here's What Actually Works` },
    { type: 'CTR', badge: 'badge-ctr', title: `Why 90% of ${cap(kw1)} Strategies Fail (And What to Do Instead)` },
    { type: 'Thought Leadership', badge: 'badge-thought', title: `The Future of ${cap(kw1)}: ${cap(kw3)} for Decision Makers` },
    { type: 'Thought Leadership', badge: 'badge-thought', title: `${cap(kw1)} in ${year}: Executive Insights and Strategic Outlook` },
    { type: 'Thought Leadership', badge: 'badge-thought', title: `Rethinking ${cap(kw1)}: A Data-Driven Leadership Perspective` },
    { type: 'Shorts Hook', badge: 'badge-shorts', title: `This ${cap(kw1)} Trick Changes Everything` },
  ]
...
TIMESTAMPS
0:00 - Introduction
1:30 - The Problem with Traditional ${cap(kw1)} Approaches
4:00 - Framework Overview
7:15 - Step-by-Step Breakdown
12:30 - Real-World Case Study
17:00 - Expert Insights & ${cap(kw3)}
21:00 - Key Takeaways & Action Steps
23:45 - Conclusion & Next Steps

RESOURCES & LINKS
Download the free ${cap(kw1)} Playbook: [LINK]
Get the ${cap(kw2)} Template: [LINK]
Visit our website for more resources: [LINK]

WATCH NEXT
${cap(kw1)} Masterclass (Full Playlist): [LINK]
${cap(kw2)} Deep Dive Series: [LINK]
Latest ${cap(kw3)} Insights: [LINK]

CONNECT WITH US
Subscribe for weekly ${cap(kw1)} insights
...
#${cap(kw1).replace(/\s/g,'')} #${cap(kw2).replace(/\s/g,'')} ${hashtagList}
