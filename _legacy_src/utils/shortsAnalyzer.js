...
export function analyzeShortsOpportunities(videoData) {
  ...
  const duration = parseDur(contentDetails?.duration || 'PT0S')
  const keywords = extractKeywords(title, description)
  const timestamps = extractTimestamps(description)
  const opportunities = generateOpportunities(timestamps, title, keywords, duration)
  return { opportunities, totalDuration: duration, timestamps }
}
...
function generateOpportunities(timestamps, title, keywords, duration) {
  const kw1 = keywords[0] || 'key insight'
  const kw2 = keywords[1] || 'strategy'
  const kw3 = keywords[2] || 'tip'

  const hooks = [
    { id:1, category:'Strong Hook', icon:'🎯', clipStart:'0:00', clipEnd:'0:58',
      ...
      viralScore:87, type:'Hook' },
    { id:2, category:'Surprising Insight', icon:'💡',
      clipStart:fmtTime(Math.floor(duration*0.2)), clipEnd:fmtTime(Math.floor(duration*0.2)+55),
      ...
      viralScore:82, type:'Insight' },
    { id:3, category:'Statistic / Data Point', icon:'📊',
      clipStart:fmtTime(Math.floor(duration*0.35)), clipEnd:fmtTime(Math.floor(duration*0.35)+45),
      hookTitle:`The ${kw1} statistic that will change how you think`,
      onScreenText:`📊 Only 12% of companies get this right`,
      caption:`This data point on ${kw1} should be in every boardroom.\n\n#DataDriven #${kw1.replace(/\s/g,'')} #BusinessIntelligence`,
      hashtags:['#DataDriven', `#${kw1.replace(/\s/g,'')}`, '#Stats', '#BusinessInsights', '#Shorts'],
      viralScore:78, type:'Statistic' },
    { id:4, category:'Executive Quote', icon:'🎤',
      clipStart:fmtTime(Math.floor(duration*0.5)), clipEnd:fmtTime(Math.floor(duration*0.5)+52),
      hookTitle:`"${cap(kw1)} is the single most underrated lever in business today"`,
      ...
      viralScore:91, type:'Quote' },
    { id:5, category:'Controversial Moment', icon:'🔥',
      clipStart:fmtTime(Math.floor(duration*0.65)), clipEnd:fmtTime(Math.floor(duration*0.65)+58),
      hookTitle:`Hot take: Why traditional ${kw2} is dead`,
      onScreenText:`Controversial opinion incoming...`,
      caption:`I said what I said about ${kw2}. Agree or disagree?\n\n#HotTake #${kw2.replace(/\s/g,'')} #FutureOf${cap(kw1).replace(/\s/g,'')}`,
      hashtags:['#HotTake', '#Debate', `#${kw2.replace(/\s/g,'')}`, '#Shorts', '#BusinessDebate'],
      viralScore:85, type:'Controversial' },
    { id:6, category:'Emotional Statement', icon:'❤️',
      clipStart:fmtTime(Math.floor(duration*0.8)), clipEnd:fmtTime(Math.floor(duration*0.8)+50),
      hookTitle:`The human side of ${kw1} that nobody talks about`,
      onScreenText:`This is why ${kw1} matters beyond the numbers`,
      caption:`Sometimes it's not just about the metrics. This moment hit differently.\n\n#Authentic #${kw1.replace(/\s/g,'')} #HumanFirst`,
      hashtags:['#Authentic', '#HumanFirst', `#${kw1.replace(/\s/g,'')}`, '#Shorts', '#Inspiration'],
      viralScore:76, type:'Emotional' },
  ]

  if (timestamps.length > 0) {
    timestamps.slice(0, 3).forEach((ts, i) => {
      if (i < hooks.length) { hooks[i].clipStart = ts.time; hooks[i].hookTitle = ts.label || hooks[i].hookTitle }
    })
  }
  return hooks
}
...
export function analyzeThumbnail(videoData) {
  ...
  const scores = { readability:13, emotionalImpact:12, textDensity:0, contrast:14, faceVisibility:12, brandingConsistency:0, ctrPotential:0 }
  const weaknesses = []
  const improvements = []
  ...
  scores.textDensity = title.length < 50 ? 14 : 10

  if (title.length >= 50) weaknesses.push('If text is overlaid on thumbnail, it may be too dense.')
  if (scores.brandingConsistency < 12) weaknesses.push('Branding consistency signals are weak - channel may not have an established visual identity yet.')
  if (likeRatio < 0.02) weaknesses.push('Low like-to-view ratio suggests the thumbnail + title combo may not be attracting the right audience.')
  ...
  return {
    thumbUrl, total, weaknesses, improvements,
    alternatives: [
      { concept:'Bold Text Overlay', description:'High-contrast background (navy or black) with 3-4 word hook text in white/yellow.', ctrBoost:'+12% estimated CTR' },
      { concept:'Face + Emotion', description:'Speaker close-up with exaggerated expression (shock/curiosity) + bright graphic element.', ctrBoost:'+18% estimated CTR' },
      { concept:'Before/After Split', description:'Split screen showing contrast (problem vs solution) with minimal text.', ctrBoost:'+15% estimated CTR' },
      { concept:'Data Visualization', description:'Clean chart or statistic graphic with brand colors - appeals to analytical audiences.', ctrBoost:'+9% estimated CTR' },
    ],
    breakdown: {
